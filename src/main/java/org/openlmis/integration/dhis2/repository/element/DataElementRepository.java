...
package org.openlmis.integration.dhis2.repository.element;
...
public interface DataElementRepository extends PagingAndSortingRepository<DataElement, UUID>,
        BaseAuditableRepository<DataElement, UUID> {

  @Query(value = "SELECT\n"
      + "    d.*\n"
      + "FROM\n"
      + "    dhis2.data_element d\n"
      + "WHERE\n"
      + "    id NOT IN (\n"
      + "        SELECT\n"
      + "            id\n"
      + "        FROM\n"
      + "            dhis2.data_element s\n"
  ...
  boolean existsByElementAndCategoryCombo(String element, String categoryCombo);
