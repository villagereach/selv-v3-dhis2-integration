...
public class ClosingBalance implements IndicatorSupplier {
  ...
  public String calculateValue(String source, Pair<ZonedDateTime, ZonedDateTime> period,
    ...
    if (source.equals(STOCKMANAGEMENT)) {
      return stockmanagementRepository.findClosingBalance(period.getSecond(), orderable, facility);
    } else if (source.equals(REQUISITION)) {
      return requisitionRepository.findClosingBalance(period.getSecond(), orderable, facility);
    } else {
